...
a root-level `prediction.txt`; the EB-NeRD archive contains a root-level `predictions.txt`, with
each system kept in its own output directory. Full retrieval outputs remain ignored unless an
explicit requirement demands otherwise.
